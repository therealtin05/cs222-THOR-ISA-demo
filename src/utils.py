...
def prompt_for_target_discovery(context):
    """Infer salient aspect/entity when target term t is not provided (extension; not in released code)."""
    new_context = f'Given the sentence "{context}", '
    prompt = new_context + 'what aspect or entity is the reviewer mainly discussing?'
    return new_context, prompt


def prompt_direct_inferring_no_target(context):
    new_context = f'Given the sentence "{context}", '
    prompt = new_context + (
        'what aspect is being discussed and what is the sentiment polarity '
